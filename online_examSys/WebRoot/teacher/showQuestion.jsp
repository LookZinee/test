...
<html lang="en">
  <head>
   <base href="<%=basePath%>">
    ...
    <script type="text/javascript">
    	function addQue(){
    		window.location="tetoaddQue.action";
    	}
    	function deleteStu(stuid){
    		var flag = confirm("您确定要删除该学生的信息吗？");
    		if(flag){
    			window.location="tedeleteQue.action?question.id="+stuid;
    		}
    	}
    	function toupdate(stuid){
    		window.location="tetoupdateQue.action?question.id="+stuid;
    	}
  ...
  <body>
       <h1 class="page-title">试题信息</h1>
<div class="btn-toolbar">
    <button class="btn btn-primary" onclick="addQue();"><i class="icon-plus"></i>添加新的试题</button>
  <div class="btn-group">
  </div>
</div>
<div class="well">
    <table class="table">
      ...
      <tbody>
      <c:forEach items="${queList }" var="each" varStatus="ea">
      	<tr>
      	 <td>${each.id}</td>
